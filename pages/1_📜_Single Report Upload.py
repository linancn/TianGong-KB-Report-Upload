...
def text_upsert(
    text_input, key_input, source_id_input, url_input, created_at_input, author_input
):
    url = "https://plugin.tiangong.world/upsert"
...
def get_zotero_collection(zot_user_id, zot_catagory, zot_api_key):
    ...
    zot_root_collection = sidebar_placeholder_root.text_input(
        "Root collection", value="MPTKQLCP"
    )
...
if zot_user_id and zot_catagory and zot_api_key:
    ...
    if "df" in st.session_state:
        ...
        if selected_indices:
            # 从Zotero获取单个PDF文件
            key_input = df.iloc[selected_indices]["key"].values[0]
            source_id_input = df.iloc[selected_indices]["title"].values[0]
            ...
            page_range = st.slider(
                "Choose pages:", min_value=1, max_value=num_pages, value=[1, num_pages]
            )
